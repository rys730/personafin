...
export default function Navbar() {
    ...
    return (
        <>
            ...
            <nav ref={navRef} className={`absolute top-0 left-0 bottom-0 h-full sm:w-16 pt-20 bg-gray-800 shadow-2xl text-clip overflow-hidden transition-all duration-200 ease-out ${open ? 'w-1/2' : 'hidden'} sm:block sm:hover:w-1/2 md:hover:w-1/4 lg:hover:w-1/4 xl:hover:w-1/5`}>
                <ul>
                {
                    Menus.map(menu=>{
                        return (
                            <li className={`flex items-center p-5 ${active === menu.name ? "bg-indigo-500" : "bg-gray-800"}`}>
